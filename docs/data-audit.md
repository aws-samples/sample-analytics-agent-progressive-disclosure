...
> ## ⚠️ 本文描述的是 v2 那批数据（Redshift + 8000 万行），**不是当前部署**
>
> 保留原文是刻意的:它是「生成数据会得什么病」的标本,也是审计方法的教材。
> 但**下面所有数字都属于一个已经不存在的数据集**,别拿它判断当前数据的质量。
>
> 当前引擎是 Athena + S3 Tables。**数据有两份,别混**:仓库里交付的是 `data/csv/` 种子
> (**48 张表约 22 万行**);而某个账号的湖里此刻装的是哪一批要单独确认 —— 本仓库开发账号
> 2026-09-17 实测已经是 **8000 万行**那一批(新生成器 `scale≈427`,由
> `feat/data-reload-80m` 的 `load_parquet.py` 灌的,**和本文这批 v2 数据同量级但不同源**)。
> 逐表数字与核查命令见 [deployment.md](deployment.md#数据说明)。
>
> 尤其注意:文末「修复后复审」那张表里被打勾的不变量,**在种子数据上多数并不成立**——
> 下面右列是 **2026-08 在当时还装着种子的湖上**实测的,不是 v2 的数,也不是重灌后的数:
>
> | 本文（v2 修复后）声称 | 22 万行种子数据实测(2026-08) |
> |---|---|
> | `posts.like_count` 不符 **0** | **1,000 / 1,000 行全部不符** |
> | `sessions.event_count` 不符 **0** | **4,932 / 5,000 行不符** |
> | purchase 事件 = 有效订单（647,265 = 647,265） | purchase 事件 **772** ≠ 有效订单 **1,601** |
> | 漏斗 max/min = 197.6（逐级衰减） | 25 类事件各自行数 max/min = **1.18**,几乎均匀 |
> | 留存 D0 → D14 单调衰减 | D0 37 / D1 38 / D3 41 / D7 30 / D14 37 / D30 23,**不单调** |
> | `user_level` 与消费强相关 | 五档均值 14,213–19,324,**看不出相关** |
> | 券核销 1.0 张/单、三元对齐 | `user_coupons.order_id` 与 `orders` **零匹配**(11,297 张券挂空) |
> | mart 对账 GMV = 151,238,025 | 91 天里 90 天 0 差异,GMV = **8,571,786.74** |
>
> ### ⚠️ 正文里有一处**推理**错误,不只是数字过期——它在 agent 身上付出过代价
>
> L5.6/L5.7 写着「事件量均匀 ⟹ 任何漏斗查询都会算出接近 100% 的转化率」,
> 开头那句「漏斗分析在这份数据上算不出有意义的结果」同源。**这个推论不成立。**
> 事件量均匀说的是每种事件**各自的行数**;漏斗要求第 i+1 步是第 i 步的**子集**,
> 加上这个约束以后即便事件量均匀,人数也会逐层掉——22 万行种子上实测
> `394 → 314 → 258 → 199`,逐层流失 20%/18%/23%,单调递减。
> 事件量均匀真正破坏的是**次序**,不是衰减:严格时序口径下末步退化到 0。
>
> 2026-08-21 agent 答漏斗题时,把「结算 396 > 浏览 394」解释成「这份种子数据不衰减」,
> 用的就是这个推论。**按约定本文正文一字不改**,所以更正只写在这里:
> 正确口径与实测见 [data-walkthrough.md](data-walkthrough.md) §4.1、
> `knowledge/analysis/funnel_analysis.md` 的约束 A,以及 `PROJECT_STATUS.md` 那条复盘。

> 当前数据的逐表实测见 [data-walkthrough.md](data-walkthrough.md);想在当前部署上重跑
> 审计,用 `scripts/lakehouse/verify_load.py`、`reconcile.py`、`verify_enums.py`、
> `verify_mart_parity.py`,**不要**跑 `scripts/audit/run.py`(那是 Redshift 路径)。

> ### L5 这一层已经有了自动化的接续,覆盖面比正文宽
>
> 正文 L5.1–L5.9 只量到 `orders` / `events` / mart 层。L5 问的那个问题（「像不像真的」）
> 现在有五个报告器接着问,其中 `scripts/lakehouse/verify_behavior.py` 专门补的是正文
> **完全没碰过**的四张行为大表:`post_likes` / `page_views` / `user_follows` /
> `push_notifications`（v2 时合计 4,058 万行 = 全库 44.45%,当前 v3 部署里 95,029 行,
> 占比同量级）。27 条判据,当前库 **21 条红**。清单、判据来源和逐项实测写在那个脚本的
> docstring 里,方法学写在 [test-plan.md](test-plan.md) 的「L2+ 数据真实性诊断」一节。
>
> 它的三条结论值得和正文对照着读,因为**正文的 L5 方法在这三处会漏**:
>
> - **「均匀」不是 0.10。** 正文判集中度时拿的是绝对数（L5.5 的 top10% GMV 41.9%）。
>   等概率抽样下 top10% 份额的期望其实是 `0.1·(1 + 1.755/√μ)` ≈ 0.12~0.13,
>   所以「实测 0.13」既可以读成「有点头部」也可以读成「纯噪声」——只有把通过线写成
>   **自算基线的倍数**才分得开。当前库四张表的集中度实测全部 = 基线的 0.96~1.33 倍。
> - **卡片声明的判定规则可以在数据上恒假。** `sessions.md:41` 的 `is_bounce` 判定表、
>   `push_notifications.md` 的「状态判断逻辑」表,三条规则在当前库上**选不出任何行**
>   （单页会话 0 个 / `failure_reason` 整列空 / `scheduled_at` 整列空）。这类缺陷
>   `verify_doc_sql.py`（只 EXPLAIN）和 `verify_enums.py`（只比取值集合）都抓不到。
> - **均匀性一旦写进卡片,就从缺陷降级成了现状。**
>   `knowledge/domains/behavior/page_views.md` 写着「30,000 行摊在 15 个页面上几乎完全
>   均匀（极差 6%）,排不出有意义的热门页面榜」。写下来是好事（agent 不会拿噪声排名当
>   结论）,副作用是**没有任何东西会因为它变红**,于是它变好或变得更糟时也没人知道。
