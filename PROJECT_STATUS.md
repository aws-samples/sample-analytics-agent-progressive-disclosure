...
- **8000 万行的生成器路径在本分支上未接**。本分支只有 `load.py`(`data/csv/` 种子 → Iceberg),
  接上全量的是 `feat/data-reload-80m` 的 `scripts/gen/main.py --target-rows 80000000` → Parquet
  → `load_parquet.py`。**注意这不等于"湖里是种子"**:那一批已经在本仓库开发账号上跑过了,湖里
  当前就是 8000 万行那一份(见 2026-09-17 那节)。本分支的代码与文档描述的是种子那一档,两者
  不一致是分支顺序造成的,不是缺陷。
