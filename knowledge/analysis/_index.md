...
⚠️ **取数题也可能要读这里**：上表里的方法有些带**口径硬约束**，不读会算错——
最典型的是漏斗（每步必须约束成上一步的子集，否则会出现「结算人数 > 浏览人数」，
本项目真踩过）。所以「转化漏斗，各步骤独立用户数」这种朴素取数题，
也要连 `funnel_analysis.md` 一起读，只是输出可以简单些（不必多图、不必 findings）。
把方法卡当成「只有深度分析题才看」，约束就永远不会生效。
