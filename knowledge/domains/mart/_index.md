...
- **归因覆盖**：渠道归因走 last_touch，只覆盖有归因记录的用户；相当一部分 GMV 落在 `channel_name='未归因'`（当前数据实测约 **64.7%**，随数据重造会变，报结论前请现算）。做渠道归因时必须把这块单列、正视它，不能假装不存在——这本身就是个值得报出来的治理发现。而**成因要说准**：其中约一半的用户只有 `first_touch` 记录、被 last_touch 口径滤掉了，不是没有渠道信息（详见 `mart_daily_revenue.md`）。
